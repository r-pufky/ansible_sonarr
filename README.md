...
Tasks are gated by feature flags and executed in the following order.
  ...
  Step | Flag                   | Notes
 ------|------------------------|-------
  1    | sonarr_flg_backup      | Backup config data. Exits role.
  2    | sonarr_flg_restore     | Restore config data. Exits role.
  3    | sonarr_flg_maintenance | Preform role maintenance tasks.
  4    | sonarr_flg_install     | Install required packages, users, etc.
  5    | sonarr_flg_config      | Install user-defined config.
...
#### New Deployments
``` yaml
# Config data: /var/opt/sonarr.
- name: 'Sonarr default install.'
...
``` yaml
- name: 'Statically deploy Sonarr with pre-existing DB and config.'
  ansible.builtin.include_role:
    name: 'r_pufky.arr.sonarr'
  vars:
...
or leave sonarr_flg_config disabled to leave existing config untouched.

``` yaml
- name: 'Upgrade Sonarr version without touching configuration.'
  ansible.builtin.include_role:
    name: 'r_pufky.arr.sonarr'
  vars:
    sonarr_srv_version: 'v4.0.17.NEW_VERSION'
    sonarr_flg_config: false
...
[f]: https://github.com/r-pufky/ansible_sonarr/blob/main/LICENSE
[g]: https://github.com/r-pufky/ansible_collection_arr
[h]: https://r-pufky.github.io/docs/media/sonarr
[i]: https://github.com/r-pufky/ansible_sonarr/blob/main/meta/main.yml
